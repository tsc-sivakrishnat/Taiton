  ...
  useEffect(() => {
    if (editId && token) {
      const loadJobDetails = async () => {
        try {
          const res = await enterpriseApi.contentList(token, 'career');
          const jobItem = res.items?.find((item) => String(item.id) === String(editId));
          if (jobItem) {
    ...
    <div className="cp-stack">
      ...
      <div className="cp-page-head">
        <div>
          ...
          <p className="cp-muted">Configure role specifications, department requirements, and sharing meta details.</p>
        </div>
      </div>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                  ...
                  <button
                    type="button"
                    className="cp-btn cp-btn-primary"
                    onClick={() => document.getElementById('og-career-file').click()}
                    style={{
                      position: 'absolute',
                      right: '4px',
                      top: '4px',
