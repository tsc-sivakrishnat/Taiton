...
export function CareersPage() {
  ...
  const [activeTab, setActiveTab] = useState(tabParam === 'applications' ? 'applications' : 'postings');
      ...
      <div className="cp-stack">
        ...
        <div className="cp-tabs" style={{ marginBottom: '16px', display: 'flex', gap: '8px', borderBottom: '1px solid var(--cp-border)', paddingBottom: '8px' }}>
          <button
            ...
            onClick={() => { setActiveTab('postings'); setSearchQuery(''); setStatusFilter(''); }}
            style={{
              padding: '8px 16px',
              borderRadius: '6px',
          ...
          <button
            ...
            onClick={() => { setActiveTab('tracking'); setSearchQuery(''); setStatusFilter(''); }}
            style={{
              padding: '8px 16px',
              borderRadius: '6px',
            ...
            {loading ? (
              ...
              const filtered = jobs.filter((row) => {
                const matchesSearch = row.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                                      (row.payload?.job_id || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
                                      (row.payload?.department || '').toLowerCase().includes(searchQuery.toLowerCase());
                const matchesStatus = !statusFilter || row.status === statusFilter;
                return matchesSearch && matchesStatus;
              });

              if (filtered.length === 0) {
                return <p className="cp-muted">No job openings match your filters.</p>;
              }

              const sliced = filtered.slice((jobsPage - 1) * 10, jobsPage * 10);
              ...
              return (
                <>
                  ...
                  <TablePagination
                    ...
                    pageSize={10}
                    labelSingle="job opening"
                    labelPlural="job openings"
                  />
                </>
              );
            })()}
          ...
          <section className="cp-card cp-card-pad">
            ...
            {loading ? (
              ...
              const filtered = applications.filter((row) => {
                const name = row.customerName || row.payload?.customer_name || '';
                const email = row.email || row.payload?.email || '';
                const jobId = row.subject || row.payload?.subject || '';
                const jobTitle = getJobTitle(jobId);
                const matchesSearch = name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                                      email.toLowerCase().includes(searchQuery.toLowerCase()) ||
                                      jobTitle.toLowerCase().includes(searchQuery.toLowerCase());
                let matchesStatus = true;
                if (statusFilter === 'live') matchesStatus = row.status === 'responded';
                else if (statusFilter === 'draft') matchesStatus = row.status === 'new';
                else if (statusFilter === 'pending_approval') matchesStatus = row.status === 'in_review';
                return matchesSearch && matchesStatus;
              });

              if (filtered.length === 0) {
                return <p className="cp-muted">No candidate applications match your filters.</p>;
              }

              const sliced = filtered.slice((appsPage - 1) * 10, appsPage * 10);
              ...
              return (
                <>
                  ...
                  <TablePagination
                    ...
                    total={filtered.length}
                    pageSize={10}
                    labelSingle="candidate application"
                    labelPlural="candidate applications"
                  />
                </>
              );
            })()}
