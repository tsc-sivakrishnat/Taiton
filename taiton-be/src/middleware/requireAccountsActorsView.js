...
/** List org actors (users) — same visibility as Accounts nav. */
export async function requireAccountsActorsView(req, res, next) {
  const r = req.auth?.role;
  if (r === 'org_admin' || r === 'manager' || r === 'super_admin' || r === 'sys_admin') {
    return next();
  }
